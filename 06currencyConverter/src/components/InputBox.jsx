...
function InputBox({
    ...
    return (
        <div className={`bg-wite p-3 rounded-lg text-sm flex ${className}`}>
        <div className='w-1/2'>
            <label htmlFor={amountInputId} className='inline-block mb-2 text-black/40'>
                {label}
            </label>
            <input 
            id={amountInputId} className='outline-none w-full bg-transparent py-1.5'
            type='number'
            placeholder='Amount'
            disabled={amountDisable}
            value={amount}
            onChange={(e) => onAmountChange && onAmountChange(Number(e.target.value))} //we get value in string that y needs to convert into number here.
            />
        </div>
